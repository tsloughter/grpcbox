...
-spec get_feature(Message :: point(), GrpcStream :: grpcbox_stream:t()) -> {ok, feature()} | grpc:error_response().
get_feature(Message, GrpcStream) ->
    self() ! {update_headers, [{<<"header">>, <<"value">>}]},
...
-spec record_route(reference(), GrpcStream :: grpcbox_stream:t()) -> route_summary() | grpc:error_response().
