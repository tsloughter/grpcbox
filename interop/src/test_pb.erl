...
decode_msg(Bin, MsgName, Opts) when is_binary(Bin) ->
    ...
    case MsgName of
      ...
      'grpc.testing.Empty' ->
	  try 'd_msg_grpc.testing.Empty'(Bin, TrUserData) catch
	    Class:Reason ->
		StackTrace = erlang:get_stacktrace(),
		error({gpb_error,
		       {decoding_failure,
			{Bin, 'grpc.testing.Empty',
			 {Class, Reason, StackTrace}}}})
	  end
    end.
...
'dfp_read_field_def_grpc.testing.Empty'(<<>>, 0, 0,
					_) ->
    #{};
'dfp_read_field_def_grpc.testing.Empty'(Other, Z1, Z2,
					TrUserData) ->
    'dg_read_field_def_grpc.testing.Empty'(Other, Z1, Z2,
					   TrUserData).

'dg_read_field_def_grpc.testing.Empty'(<<1:1, X:7,
					 Rest/binary>>,
				       N, Acc, TrUserData)
    when N < 32 - 7 ->
    'dg_read_field_def_grpc.testing.Empty'(Rest, N + 7,
					   X bsl N + Acc, TrUserData);
'dg_read_field_def_grpc.testing.Empty'(<<0:1, X:7,
					 Rest/binary>>,
				       N, Acc, TrUserData) ->
    Key = X bsl N + Acc,
    case Key band 7 of
      0 ->
	  'skip_varint_grpc.testing.Empty'(Rest, 0, 0,
					   TrUserData);
      1 ->
	  'skip_64_grpc.testing.Empty'(Rest, 0, 0, TrUserData);
      2 ->
	  'skip_length_delimited_grpc.testing.Empty'(Rest, 0, 0,
						     TrUserData);
      3 ->
	  'skip_group_grpc.testing.Empty'(Rest, Key bsr 3, 0,
					  TrUserData);
      5 ->
	  'skip_32_grpc.testing.Empty'(Rest, 0, 0, TrUserData)
    end;
'dg_read_field_def_grpc.testing.Empty'(<<>>, 0, 0, _) ->
    #{}.

'skip_varint_grpc.testing.Empty'(<<1:1, _:7,
				   Rest/binary>>,
				 Z1, Z2, TrUserData) ->
    'skip_varint_grpc.testing.Empty'(Rest, Z1, Z2,
				     TrUserData);
'skip_varint_grpc.testing.Empty'(<<0:1, _:7,
				   Rest/binary>>,
				 Z1, Z2, TrUserData) ->
    'dfp_read_field_def_grpc.testing.Empty'(Rest, Z1, Z2,
					    TrUserData).

'skip_length_delimited_grpc.testing.Empty'(<<1:1, X:7,
					     Rest/binary>>,
					   N, Acc, TrUserData)
    when N < 57 ->
    'skip_length_delimited_grpc.testing.Empty'(Rest, N + 7,
					       X bsl N + Acc, TrUserData);
'skip_length_delimited_grpc.testing.Empty'(<<0:1, X:7,
					     Rest/binary>>,
					   N, Acc, TrUserData) ->
    Length = X bsl N + Acc,
    <<_:Length/binary, Rest2/binary>> = Rest,
    'dfp_read_field_def_grpc.testing.Empty'(Rest2, 0, 0,
					    TrUserData).

'skip_group_grpc.testing.Empty'(Bin, FNum, Z2,
				TrUserData) ->
    {_, Rest} = read_group(Bin, FNum),
    'dfp_read_field_def_grpc.testing.Empty'(Rest, 0, Z2,
					    TrUserData).

'skip_32_grpc.testing.Empty'(<<_:32, Rest/binary>>, Z1,
			     Z2, TrUserData) ->
    'dfp_read_field_def_grpc.testing.Empty'(Rest, Z1, Z2,
					    TrUserData).

'skip_64_grpc.testing.Empty'(<<_:64, Rest/binary>>, Z1,
			     Z2, TrUserData) ->
    'dfp_read_field_def_grpc.testing.Empty'(Rest, Z1, Z2,
					    TrUserData).
...
'v_msg_grpc.testing.Empty'(#{} = M, Path, _) ->
    lists:foreach(fun (OtherKey) ->
			  mk_type_error({extraneous_key, OtherKey}, M, Path)
		  end,
		  maps:keys(M)),
    ok;
'v_msg_grpc.testing.Empty'(M, Path, _TrUserData)
    when is_map(M) ->
    mk_type_error({missing_fields, [] -- maps:keys(M),
		   'grpc.testing.Empty'},
		  M, Path);
'v_msg_grpc.testing.Empty'(X, Path, _TrUserData) ->
    mk_type_error({expected_msg, 'grpc.testing.Empty'}, X,
		  Path).
...
get_service_def('grpc.testing.TestService') ->
    {{service, 'grpc.testing.TestService'},
     [#{name => 'EmptyCall', input => 'grpc.testing.Empty',
	output => 'grpc.testing.Empty', input_stream => false,
	output_stream => false, opts => []},
      #{name => 'UnaryCall',
	input => 'grpc.testing.SimpleRequest',
	output => 'grpc.testing.SimpleResponse',
	...
	input => 'grpc.testing.StreamingOutputCallRequest',
	output => 'grpc.testing.StreamingOutputCallResponse',
	input_stream => false, output_stream => true,
	opts => []},
      #{name => 'StreamingInputCall',
	input => 'grpc.testing.StreamingInputCallRequest',
	output => 'grpc.testing.StreamingInputCallResponse',
...
get_service_def('grpc.testing.ReconnectService') ->
    {{service, 'grpc.testing.ReconnectService'},
     [#{name => 'Start',
	input => 'grpc.testing.ReconnectParams',
	output => 'grpc.testing.Empty', input_stream => false,
	output_stream => false, opts => []},
      #{name => 'Stop', input => 'grpc.testing.Empty',
	output => 'grpc.testing.ReconnectInfo',
	input_stream => false, output_stream => false,
	opts => []}]};
